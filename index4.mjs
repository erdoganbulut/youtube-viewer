...
const MAX_CONCURRENT_BROWSERS = 34;
...
async function processProxy(proxy, index) {
  ...
  try {
    ...
    const button = await page.waitForSelector('.ytSearchb element.isoxComponentSearchButton');
    await button.click();
    await new Promise(resolve => setTimeout(resolve, 5000));
...
(async () => {
  ...
  const browserPromises = list.slice(30, MAX_CONCURRENT_BROWSERS).map(async (proxy, index) => {
